...
var Zepto = (function() {
  ...
  function defaultDisplay(nodeName) {
    var element, display
    if (!elementDisplay[nodeName]) { //其上部定义的elementDisplay缓存里是否有这个节点值
  ...
  zepto.init = function(selector, context) {
    ...
    if (!selector) return zepto.Z() //没有参数，返回空数组
      ...
      if (selector[0] == '<' && fragmentRE.test(selector)) {
        dom = zepto.fragment(selector, RegExp.$1, context), selector = null //得到这个dom的集合，并把selector手动重置
      }
  ...
  zepto.qsa = function(element, selector) {
    ...
    return (isDocument(element) && isSimple && maybeID) ?
      ...
      slice.call(
        isSimple && !maybeID ?
        maybeClass ? element.getElementsByClassName(nameOnly) : // If it's simple, it could be a class 如果selector是class名,直接调用getElementsByClassName
...
(function($) {
  ...
  function findHandlers(element, event, fn, selector) {
    ...
    return (handlers[zid(element)] || []).filter(function(handler) {
      //判断事件类型是否相同
      return handler && (!event.e || handler.e == event.e) && (!event.ns || matcher.test(handler.ns)) && (!fn || zid(handler.fn) === zid(fn)) && (!selector || handler.sel == selector) //注意函数是引用类型的数据zid(handler.fn)的作用是返回handler.fn的标示符，如果没有，则给它添加一个，
  ...
  function parse(event) {
    ...
    return {
      ...
      ns: parts.slice(1).sort().join(' ') //其命名空间
    }
  }
...
(function($) {
  ...
  function touchLeft() {
    ...
    var X = 0;
    var objX = 0;
    $(document).on("touchstart", '.z-ecp-li', function(e) {
      e.preventDefault();
      var obj = this;
        ...
        // for instance when moving to a point and back to origin
          if (deltaX < 30 && deltaY < 30) {
            ...
            tapTimeout = setTimeout(function() {
              ...
              console.debug(Number(Date.now()) - _$e);
            }, 3)
          } else {
            touch = {}
          }
