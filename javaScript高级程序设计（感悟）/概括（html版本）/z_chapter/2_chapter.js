...
(function() {
    var source1 = '<div class="page-header">' +
        ...
        '<p><%=subStr[i].extra_content[j].content%></p>' +
        '<% } %>' +
        '</blockquote>' +
        '<% } %>'+
        ...
        '<p><%=subStr[i].subStr[j].extra_content[n].content%></p>' +
        '<% } %>' +
        '</blockquote>' +
        '<% } %>' +
        '<% } %>' +
    ...
    article.Render(source1, second_chapter, 'content');
    ...
    var nav = new zDoc();
    nav.Render(source2, second_chapter, 'accordion');
})();
